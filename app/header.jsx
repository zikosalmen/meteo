...
import Recherche from "./comps/recherche"
...
export default function Header() {
  ...
  const langue = Array.isArray(param?.local)
    ? param.local[0]
    : param?.local || "ar";
...
useEffect(() => {
  ...
  return (
    <header
      className={`flex flex-col sm:flex-row items-center justify-between gap-1 sm:gap-3 
              backdrop-invert p-3 
              ${
                dark
    ...
    >
      <Link href="/" className="flex items-center gap-2 sm:gap-3">
        ...
        />
        <p className="text-lg sm:text-xl font-bold">{t("meteo") + ciel}</p>
      </Link>
      ...
      <span
        className={`flex items-center gap-1 sm:gap-2 py-1 px-3 rounded text-sm
                   backdrop-invert backdrop-opacity-5 ${
                     dark ? "text-white bg-black/30" : "bg-white/30 text-black"
                   }`}
      >
        <select
          name=""
